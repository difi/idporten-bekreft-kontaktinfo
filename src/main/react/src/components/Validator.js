...
class Validator extends Component {
    ...
    validateMobile(value, valueRepeat = null, oldValue = null){

        if(value.length){
            // check for number starting with 4 or 9, optional landcode (0047,+47) and min 8 digits, max 20
            if (!value.replace(/\s+/g, '').match("^([0]{2}[0-9]{2})?([+][0-9]{2})?[49][0-9]{7,19}$")){
                return this.MOBILE_ERROR;
            }
        }

        if (valueRepeat && value !== valueRepeat){
            return this.MOBILE_REPEAT_ERROR;
        }
    }

    validateEmail(value, valueRepeat = null, oldValue = null){
    ...
    /* custom for idk */
    isMobileRemoved(value, valueRepeat = null, oldValue = null){
        if(oldValue){
            if(oldValue !== "" && value === "" || oldValue !== "" && valueRepeat === ""){
